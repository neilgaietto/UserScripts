...
$(document).ajaxComplete(function() { 
    ...
    if(!loaded){
        ...
        //expand all
        var btnExpandAll = $("<a href='#'>Expand All</a>");
        $('.filters.top-filters.search-bar').append(btnExpandAll);
        btnExpandAll.one( "click", function(){
            var expandAll = function () { $(".toggler.substory-toggle:not(.expanded):not(.working):not(.invisible)").each(function () { $(this).addClass('working').click(); }); }
            $(document).ajaxComplete(function () { expandAll() });
            expandAll();
        });
    }
